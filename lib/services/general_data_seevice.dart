...
class GeneralDataService {
  ...
  static Future<bool> initServiceAndCounterData() async {
    activeServices.clear();
    activeCounters.clear();
    String _path = 'get/call/page';
    var response = await NetworkingService.getHttp(_path);
    if (response is Response &&
        response.statusCode == 200 &&
        response.data['data'] != null) {
      var services =
  ...
  static Future<void> createNewServiceAndCounterTab({
    ...
    String _counterString = counter.name;
    String _servicesString = services
        .map<String?>((service) {
          String? ser;
          try {
            ser = service.name;
    ...
    ServiceCounterTabModel newTab = ServiceCounterTabModel(
      ...
      counterSettings: CounterSettingsModel.generateDefaultSettings(),
      selected: _allServiceCounterTabs.isEmpty ? true : false,
      serviceString: _servicesString,
      counterString: _counterString,
    );
    _allServiceCounterTabs.add(newTab);
    if (_allServiceCounterTabs.length == 1) {
      currentServiceCounterTabIndex = 0;
  ...
  static Future<void> selectThisTab({
    ...
    if (_currentIndex >= 0) {
      _allServiceCounterTabs[_currentIndex] = ServiceCounterTabModel(
        services: _allServiceCounterTabs[_currentIndex].services,
        counter: _allServiceCounterTabs[_currentIndex].counter,
        counterSettings: _allServiceCounterTabs[_currentIndex].counterSettings,
        selected: false,
        serviceString: _allServiceCounterTabs[_currentIndex]
            .serviceString
            .replaceAll("*", ""),
        counterString: _allServiceCounterTabs[_currentIndex].counterString,
      );
    }
    ...
    await getLastToken();
    await getTodayTokenDetails();
    SetDeviceService.addCounterAppDetails();
    // await reloadData();
  }
  ...
  static Future<ServiceCounterTabModel> checkTabValidity(
      {required ServiceCounterTabModel thisTab}) async {
    List<int> availableServices = activeServices.map((e) => e.id).toList();
    List<int> availableCounters = activeCounters.map((e) => e.id).toList();
    List<ServiceModel> newTabServices = [];
    List<CounterModel> newTabCounter = [];
    if (availableCounters.contains(thisTab.counter.id)) {
      newTabCounter.add(activeCounters
          .firstWhere((element) => element.id == thisTab.counter.id));
    }
    for (var item in thisTab.services) {
      if (availableServices.contains(item.id)) {
        newTabServices
    ...
    ServiceCounterTabModel newTab = ServiceCounterTabModel(
      ...
      serviceString: newTabServices
          ...
          .toList()
          .join(', '),
      counterString: newTabCounter.map<String?>((counter) {
        String? ser;
        try {
          ser = counter.name;
        } catch (e) {
  ...
  static Future<void> _getTabDetailsFromStorage() async {
    ...
    if (counterTabsString != null) {
      ...
      for (var _item in json.decode(counterTabsString)) {
        var _tab =
            ServiceCounterTabModel.fromJson(_item as Map<String, dynamic>);
        _allServiceCounterTabs.add(_tab);
        if (_tab.selected) {
          currentServiceCounterTab = _tab;
        }
        currentServiceCounterTabIndex = _allServiceCounterTabs
                    .indexWhere((element) => element.selected) >=
                0
  ...
  static Future<Map<String, dynamic>> checkTabAlreadyFound({
    ...
    List _selectedServices = services.map((e) => e.id).toList();
    _selectedServices.sort();
    int _index = _allServiceCounterTabs.indexWhere((element) {
      List _currentElementServices = element.services.map((e) => e.id).toList();
      _currentElementServices.sort();
      return listEquals(_currentElementServices, _selectedServices);
    });
    if (_index >= 0 &&
        _allServiceCounterTabs[_index].counter.id == counter.id) {
      return {'status': true, 'index': _index};
    }
    return {'status': false, 'index': -1};
  }
    ...
    if (tokenModel != null) {
      ...
      if (_allServiceCounterTabs.isNotEmpty &&
          _allServiceCounterTabs
              .asMap()
              .containsKey(currentServiceCounterTabIndex)) {
        const String _path = 'call/get/last_token';
        var response = await NetworkingService.postHttp(path: _path, data: {
          'counter':
              _allServiceCounterTabs[currentServiceCounterTabIndex].counter.id,
          'services': _allServiceCounterTabs[currentServiceCounterTabIndex]
              .services
        ...
        if (response is Response && response.statusCode == 200) {
          if (response.data['data'] != null) {
            TokenModel _lastCalledToken = TokenModel.fromEntity(
                TokenEntity.fromJson(response.data['data']));
            lastCalledToken = _lastCalledToken;
          }
        }
      }
      return false;
  ...
  static Future<void> queueCancelled(QueueModel queue) async {
    /// REMOVING FROM TODAYSQUEUE(NEXT TO CALL), IF NO ITEM FOUND WILL THROW AN EXCEPTION,
    /// SO NO ACTION IN EFFECT
    try {
      todaysQueue.removeAt(
          todaysQueue.indexWhere((element) => element.id == queue.id));
    } catch (_) {}

    ///REMOVING FROM CANCELLED QUEUE,IF NO ITEM FOUND WILL THROW AN EXCEPTION,
    ///SO NO ACTION IN EFFECT
    try {
      todaysQueueCancelled.removeAt(
          todaysQueueCancelled.indexWhere((element) => element.id == queue.id));
    ...
    ///REMOVING FROM HOLDED QUEUE, IF NO ITEM FOUND WILL THROW AN EXCEPTION,
    ///SO NO ACTION IN EFFECT
    try {
      holdedQueue.removeAt(
          holdedQueue.indexWhere((element) => element.id == queue.id));
    ...
    ///ADDING TO CANCELLED QUEUE
    todaysQueueCancelled.add(queue);
  }

  ///update functions for data varibales.
  ///
  static Future<void> updateTransferTokenDetails(TokenModel token,
      {Map<String, dynamic>? transferTO}) async {
    if (transferTO != null) {
